...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Cierres de turno — {{ $report['dateFrom'] }} a {{ $report['dateTo'] }}</p>
        </div>
        <div class="content">
            <p>Sucursal: <strong>{{ $report['branchName'] }}</strong></p>
            <div class="stat-row"><span class="stat-label">Cierres</span><span class="stat-value">{{ $report['count'] }}</span></div>
            <div class="stat-row"><span class="stat-label">Diferencia acumulada</span><span class="stat-value">{{ $report['totalDifference'] >= 0 ? '+' : '-' }}${{ number_format(abs($report['totalDifference']), 2) }}</span></div>
