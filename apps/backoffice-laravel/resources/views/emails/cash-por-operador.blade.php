...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Por operador — {{ $report['dateFrom'] }} a {{ $report['dateTo'] }}</p>
        </div>
        <div class="content">
            <p>Sucursal: <strong>{{ $report['branchName'] }}</strong></p>
            @foreach($report['byOperator'] as $op)
                <div class="stat-row"><span class="stat-label">{{ $op['name'] }} ({{ $op['count'] }})</span><span class="stat-value">+${{ number_format($op['totalEntradas'], 2) }} / -${{ number_format($op['totalSalidas'], 2) }}</span></div>
            @endforeach
