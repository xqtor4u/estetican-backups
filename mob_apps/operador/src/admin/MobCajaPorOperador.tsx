...
/**
 * Tercer reporte real de Caja — quién registró cada movimiento. Entradas/salidas separadas
 * por operador (una reversión la registra quien la revierte, no necesariamente el mismo
 * operador del movimiento original — mezclarlas en un solo monto sería engañoso). Consume
 * `/api/cash/reports/por-operador`, ya agregado del lado del servidor.
 */
interface OperatorGroup { name: string; count: number; totalEntradas: number; totalSalidas: number }
interface PorOperadorData {
  byOperator: OperatorGroup[];
...
export function MobCajaPorOperador() {
  ...
  const [result,   setResult]   = useState<PorOperadorData | null>(null);
  ...
  const fetchReport = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/cash/reports/por-operador?${queryParams()}`);
          ...
          <>
            <div className="bg-surface-container rounded-2xl overflow-hidden">
              <div className="px-4 py-2.5 border-b border-outline-variant">
                <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest">Movimientos por operador</p>
              </div>
              {result.byOperator.length === 0 ? (
                <div className="px-4 py-6 text-center">
                  <p className="text-sm text-on-surface-variant">Sin movimientos para este período</p>
                </div>
              ) : (
                result.byOperator.map((op, i) => (
                  <div key={op.name}
                    className={`px-4 py-3 ${i < result.byOperator.length - 1 ? 'border-b border-outline-variant' : ''}`}>
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-sm font-medium text-on-surface">{op.name}</p>
                      <p className="text-[11px] text-on-surface-variant">{op.count} mov.</p>
                    </div>
                    <div className="flex gap-4">
                      <p className="text-xs text-primary font-semibold">+{fmtMoney(op.totalEntradas)}</p>
                      <p className="text-xs text-error font-semibold">-{fmtMoney(op.totalSalidas)}</p>
                    </div>
                  </div>
                ))
              )}
            ...
            <ReportActionsBar
              pdfUrl={`/api/cash/reports/por-operador/pdf?${queryParams()}`}
              emailUrl={`/api/cash/reports/por-operador/email?${queryParams()}`}
              filename={`por-operador-${dateFrom}-a-${dateTo}.pdf`}
