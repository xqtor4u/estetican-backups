...
@section('content')
<x-page-header eyebrow="Agenda Hotel" :title="'Reserva hotel de ' . ($hotelReservation->pet?->name ?: 'mascota')" subtitle="Reserva planeada con control de jaula bloqueada.">
    <x-slot:actions>
        <a href="{{ route('hotel-reservations.index') }}" class="btn btn-outline-secondary">Volver al listado</a>
        <a href="{{ route('hotel-reservations.edit', $hotelReservation) }}" class="btn btn-secondary">Editar reserva</a>
        ...
        <div class="card h-100">
            <div class="card-body">
                <dl class="row mb-0">
                    ...
                    <dd class="col-sm-8">{{ $hotelReservation->start_at?->format('d/m/Y H:i') }}</dd>

                    <dt class="col-sm-4">Fin</dt>
                    <dd class="col-sm-8">{{ $hotelReservation->end_at?->format('d/m/Y H:i') }}</dd>
        ...
        <div class="card h-100">
            <div class="card-body">
                ...
                @if($resourceAllocation)
                    ...
                    <p class="mb-2"><strong>Ventana:</strong> {{ $resourceAllocation->starts_at?->format('d/m/Y H:i') }} - {{ $resourceAllocation->ends_at?->format('d/m/Y H:i') }}</p>
