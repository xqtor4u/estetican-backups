...
>
    <x-slot:actions>
        <a href="{{ route('pets.index') }}" class="btn btn-outline-dark">Programar desde mascota</a>
    </x-slot:actions>
</x-page-header>
...
<div class="catalog-content-wide">
    <section class="catalog-overview mb-4">
        <div class="catalog-overview__grid">
            <article class="catalog-overview-card catalog-overview-card--primary">
                <span class="catalog-overview-card__eyebrow">Agenda SPA</span>
                <div class="catalog-overview-card__value">{{ $bookings->total() }}</div>
                <p class="catalog-overview-card__text">{{ $operationalDateLabel }} con foco en sesiones puntuales, carga visible y lectura operativa por horario.</p>
            </article>
            ...
            <article class="catalog-overview-card">
                ...
                <div class="catalog-overview-card__value-sm">
                    @if($firstScheduledAt && $lastScheduledEndAt)
                        {{ $firstScheduledAt->format('H:i') }} - {{ $lastScheduledEndAt->format('H:i') }}
                    @else
                        Sin carga visible
                    @endif
                </div>
    ...
    <section class="agenda-scope-switch mb-3" aria-label="Ventana operativa de agenda SPA">
        ...
        <span class="agenda-scope-switch__hint">Agenda actual afinada para SPA. Hotel y otras líneas seguirán reglas propias.</span>
    </section>
    ...
    <x-list-filters :action="route('agenda.index')" :reset-url="route('agenda.index')">
        <div class="col-lg-3 col-md-6">
            ...
            <div class="form-text">Cruza mascota, cliente y catálogo programado en una sola búsqueda.</div>
        </div>
        <div class="col-lg-3 col-md-6">
    ...
    <section class="card shadow-sm border-0 mb-4">
        <div class="card-body p-4">
            ...
            <div class="agenda-timeline">
                @forelse($timelineBookings as $booking)
                    <article class="agenda-timeline-item">
                        <div class="agenda-timeline-item__time">
                            <div class="agenda-timeline-item__time-main">{{ $booking->time_window_label ?? $booking->scheduled_at?->format('H:i') }}</div>
                            <div class="agenda-timeline-item__time-sub">{{ $booking->estimated_duration_minutes }} min estimados</div>
                        </div>
                        <div class="agenda-timeline-item__body">
                            <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                                <div>
                                    <div class="catalog-title-stack__title">{{ $booking->pet?->name ?: 'Mascota sin nombre' }}</div>
                ...
                @empty
                    <div class="text-center py-4 text-body-secondary">No hay bloques horarios visibles para esta ventana operativa.</div>
                @endforelse
            </div>
        </div>
    </section>
