...
| `scheduled_offset_minutes` | unsigned smallint, default 0 | `SYNC-068` (29/08/2026, portado desde `tst`) — minutos desde `spa_bookings.scheduled_at` en que arranca esta línea. `0` = pegada al inicio. Permite huecos entre servicios / hora de inicio por línea en el agendado móvil (`MobCitaNueva`). Retrocompatible: líneas viejas quedan en `0`. `Api\BookingController::store` la acepta como `services.*.offset_minutes` (0..960); `OperatorAvailabilityChecker::hasConflict` usa `[scheduled_at + offset, + duración)` por línea |
| `duration_minutes` | unsigned smallint nullable | `SYNC-068` — duración real de esta línea al agendar (editable, stepper ±5). `NULL` = usar la del catálogo (`services.duration_minutes`). Antes `SYNC-043` solo la calculaba para validar; ahora **se persiste**. `spa_bookings.duration_minutes` de una cita con líneas = `max(offset + duración)` de sus líneas activas (fin más lejano, no la suma) — recalculado por `BookingController::store/update` y `ServiceLineActionService` al cancelar/no-realizar/reactivar |
...
**Traslape de horario por operador:** `App\Domain\Planning\Services\OperatorAvailabilityChecker::hasConflict()` (excluye `cancelled`/`no_show`). Alcance intencional: solo SPA, `operator_id` no existe en `hotel_reservations`. A partir de esta validación, elegir operador es **obligatorio** para agendar una cita SPA (antes era opcional/inexistente en el flujo web). **`SYNC-068` (29/08/2026) — reescrito para no dejar "zombies":** ahora mira (a) cada **línea de `spa_booking_services`** asignada a ese operador (activa — sin `cancelled_at`/`not_performed_at`), cada una en su tramo `[scheduled_at + scheduled_offset_minutes, + duración de línea)`, y (b) las citas **sin líneas** (solo encabezado) contra `spa_bookings.operator_id` + `duration_minutes`. Así un operador que hace un solo servicio dentro de una cita ajena sí aparece ocupado, y cancelar una línea libera su tramo. `validateSequentialAssignments` respeta el `offset_minutes` por línea (si no viene, la línea sigue pegada al fin de la anterior). `Api\AgendaController::index` con `?operator_id=` devuelve además `busy: [{start,end}]` = los tramos reales del operador consultado, uno por línea activa.
